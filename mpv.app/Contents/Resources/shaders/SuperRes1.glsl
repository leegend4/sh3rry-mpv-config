...
vec4 hook() {
    ...
    return vec4(Gamma(avg.xyz), Luma(avg.xyz));
}
...
#define GetY(x,y)   ( LOWRES_tex(ddxddy*(pos+vec2(x,y)+0.5)).a )
//Downsampled result
#define Lowres(x,y) ( LOWRES_tex(ddxddy*(pos+vec2(x,y)+0.5)).xyz )
...
vec4 hook() {
    ...
    for (int Y = minX; Y <= maxX; Y++)
    {
        ...
        diff += weight*(Lowres(X,Y) - Gamma(PREKERNEL_tex(ddxddy*(pos+vec2(X,Y)+0.5)).xyz));
        weightSum += weight;
    }
    diff /= weightSum;
    c0.xyz = Gamma(c0.xyz);
